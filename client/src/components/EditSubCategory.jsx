...
const EditSubCategoryModel = ({ close, data,fetchData }) => {
  ...
  name: data.name,
  image: data.image,
  category: data.category_id ? [{ id: data.category_id, name: data.category_name }] : [],
});
  ...
  try {
    ...
    const response = await Axios({
      ...
      data: {
        ...
        category_id: subCategoryData.category[0]?.id,
      },
    });
            ...
            <div className="border focus-within:border-primary-200 rounded">
              ...
              <div className="flex flex-wrap gap-2">
                {subCategoryData.category.map((cat, index) => {
                  return (
                    <div
                      key={(cat?.id || index) + "selectedValue"}
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setSubCategoryData((preve) => {
                    if (!categoryDetails) return preve;
                    return {
                      ...preve,
              ...
              >
                ...
                {allCategory.map((category, index) => {
                  return (
                    <option
                      value={category?.id}
                      key={(category?.id || index) + "subcategory"}
                    >
                      {" "}
                      {category?.name}
                    </option>
