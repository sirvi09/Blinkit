...
const ProductListPage = () => {
  ...
  useEffect(() => {
    const sub = AllSubCategory.filter((s) => {
      return Number(s.category_id) === Number(categoryId);
    });
        ...
        <div className="min-h-[88vh] max-h-[88vh] overflow-y-scroll grid gap-1 shadow-md scrollbarCustom bg-white py-2">
          {DisplaySubCatory.map((s, index) => {
            const link = `/${valideURLConvert(
              s.category_name,
            )}-${s.category_id}/${valideURLConvert(s.name)}-${s.id}`;

            return (
              <Link
                key={s.id + "subcategory" + index}
                to={link}
                className={`w-full p-2 lg:flex items-center lg:w-full lg:h-16 box-border lg:gap-4 border-b 
                  hover:bg-green-100 cursor-pointer
                  ${
                    Number(subCategoryId) === Number(s.id) ? "bg-green-100" : ""
                  }
                `}
              >
